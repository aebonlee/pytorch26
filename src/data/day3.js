  ...
  sessions: [
    {
      ...
      theory: [
        {
          h: '연속 행동 공간의 문제',
          p: `로봇 관절 토크, 자동차 핸들 각도처럼 행동이 연속값이면 max_a Q(s,a)를 계산할 수 없습니다 — 행동이 무한히 많기 때문입니다. 행동을 이산화하면 차원이 조금만 커져도 조합이 폭발합니다(7자유도 로봇 × 10단계 = 10^7 행동).`,
        },
        {
          h: '결정적 정책 경사 (DPG)',
          p: `해법: max를 "학습"으로 대체합니다. 상태를 받아 연속 행동을 직접 출력하는 결정적 정책 μ(s;θ)를 두고, Critic이 평가한 Q(s, μ(s))가 커지는 방향으로 Actor를 갱신합니다.
...
def soft_update(target, source, tau=0.005):
    ...
    },
    {
      ...
      theory: [
        {
          h: '실습 개요',
          p: `Pendulum-v1(진자 세우기, 행동 = 토크 1차원 연속값, 보상 최대 0)에서 DDPG를 완성합니다. 리턴이 -200 근처까지 오르면 성공적으로 학습된 것입니다.`,
        },
      ],
      code: {
...
결정적 정책은 하나의 해에 조기 수렴하기 쉽습니다. 탐험이 부족해지고, 환경이 조금만 바뀌어도 무너지며, 동일하게 좋은 여러 경로가 있어도 하나만 배웁니다.`,
        },
        {
          h: '최대 엔트로피 목적함수',
          p: `J(π) = E[ Σ r_t + α·H(π(·|s_t)) ]
매 스텝 정책의 엔트로피 H를 보상에 더합니다. "보상을 최대화하되, 가능한 한 무작위로 행동하라"는 뜻입니다.
...
soft 정책 개선의 최적해는 π(a|s) ∝ exp(Q(s,a)/α) — Q값이 높은 행동일수록 지수적으로 자주 선택하되 결코 확률 0이 되지 않는 볼츠만 분포입니다. 이 이론 위에 세워진 실용 알고리즘이 다음 교시의 SAC입니다.`,
        },
      ],
      code: {
...
③ 자동 온도 조절: α도 학습합니다. 목표 엔트로피(보통 −action_dim)를 정해 두고, 현재 정책의 엔트로피가 그보다 낮으면 α를 키워 탐험을 늘리는 방향으로 자동 조정됩니다.`,
        },
        {
          h: '재매개변수화 트릭',
          p: `Actor 손실 E[ α log π(a|s) − Q(s,a) ]를 줄이려면 샘플링 연산을 통과해 기울기를 흘려야 합니다.
a = tanh( μ(s) + σ(s) ⊙ ε ),  ε ~ N(0, I)
무작위성을 파라미터와 무관한 ε으로 분리하면 μ, σ에 대해 미분 가능해집니다 — VAE에서 쓰는 것과 같은 트릭입니다. PyTorch에서는 dist.rsample()이 이 역할을 합니다 (sample()과의 차이에 주의!).`,
        },
      ],
      code: {
...
class GaussianActor(nn.Module):
    ...
    },
    {
      ...
      theory: [
        {
          h: '샤논 엔트로피의 한계',
          p: `SAC의 softmax 정책 π ∝ exp(Q/α)는 모든 행동에 0이 아닌 확률을 부여합니다. Q값이 명백히 낮은 나쁜 행동도 계속 시도한다는 뜻입니다. 행동 공간이 크거나 위험한 행동이 있는 문제에서는 이 "끝까지 남는 꼬리 확률"이 성능을 갉아먹습니다.`,
        },
        {
...
q가 하나의 다이얼이 됩니다: q=1(SAC, 모든 행동 탐험) ↔ q=2(유망한 행동에만 집중 탐험).
효과: 행동 공간이 크고 보상이 밀집된 문제에서 나쁜 행동에 낭비하는 샘플이 줄어 학습이 빨라집니다. 반대로 sparse reward 문제라면 넓은 탐험(q≈1)이 유리할 수 있습니다 — 문제 특성에 맞게 엔트로피의 "모양"을 선택한다는 관점이 핵심입니다.`,
        },
      ],
      code: {
...
# q=2.0 → 나쁜 행동(Q=-1)의 확률이 사실상 0으로 — sparse한 탐험`,
    ...
    },
    {
      ...
      theory: [
        {
          h: '실습 개요',
          p: `4교시의 GaussianActor와 DDPG 실습의 Critic·버퍼·soft_update를 재사용해 SAC를 조립합니다. 트윈 Q와 자동 α 조절이 새로 추가되는 부분입니다. DDPG보다 적은 에피소드로 안정적으로 수렴하는 것을 확인하세요.`,
        },
      ],
      code: {
...
for episode in range(150):
    ...
    },
    {
      ...
      theory: [
        {
          h: '실습 개요',
          p: `TAC의 아름다운 점: SAC 구현에서 log π를 log_q π로 바꾸는 것이 수정의 전부입니다. q=1.0으로 두면 SAC와 완전히 동일하게 동작하는지 먼저 검증하고(회귀 테스트), q=1.5, 2.0으로 올려 학습 곡선을 비교합니다.
