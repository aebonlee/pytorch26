...
두 네트워크가 같은 방향으로 동시에 과대평가할 확률은 낮으므로 편향이 크게 줄어듭니다.\n코드 수정은 한 줄이지만 아타리 벤치마크 성능이 유의미하게 오릅니다.`,
        },
        {
...
# 평균 리턴이 475를 넘으면 CartPole 해결!`,
    ...
    },
    {
      ...
      theory: [
        {
          h: '정책을 직접 학습하기',
          p: `DQN은 Q를 배우고 정책은 argmax로 유도했습니다.\n정책 기반 방법은 정책 자체를 신경망 π(a|s;θ)로 만들고 기대 리턴 J(θ)를 직접 최대화합니다.
...
상태에만 의존하는 베이스라인 b(s)를 빼도 기대값은 불변이지만 분산은 크게 줄어듭니다.\nb(s) = V(s)를 쓰면 (G_t − V(s))는 "그 행동이 평균보다 얼마나 좋았나"를 뜻하는 어드밴티지가 됩니다 — 다음 교시 Actor-Critic의 핵심 아이디어입니다.`,
        },
      ],
      code: {
...
1일차의 "MC → TD" 전환을 정책 경사에 그대로 적용한 것입니다.`,
        ...
        {
          h: '어드밴티지와 A2C',
          p: `어드밴티지 A(s,a) = Q(s,a) − V(s)는 "이 행동이 그 상태의 평균보다 얼마나 좋은가"입니다.\n실전에서는 TD 오차 δ = r + γV(s') − V(s)가 어드밴티지의 불편추정치이므로 Critic 하나로 충분합니다.
Actor 손실:  −log π(a|s) · δ  (δ는 상수 취급, detach)
Critic 손실: δ²  (MSE)
A2C(Advantage Actor-Critic)는 여기에 여러 환경 병렬 수집 + 엔트로피 보너스(탐험 유지)를 더한 표준 구현입니다.\nA3C의 동기(synchronous) 버전이기도 합니다.`,
        },
      ],
      code: {
...
def a2c_loss(logits, value, action, td_target, entropy_coef=0.01):
    ...
    },
    {
      ...
      theory: [
        {
          h: '실습 개요',
          p: `앞 교시의 ActorCritic 네트워크로 전체 학습 루프를 완성합니다.\nn-step(여기서는 5스텝) 롤아웃마다 업데이트해 REINFORCE보다 빠르게, DQN보다 부드럽게 학습되는 것을 확인합니다.`,
        },
      ],
      code: {
