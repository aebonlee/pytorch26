...
"확률의 기울기"를 직접 구하는 대신 "로그 확률의 기울기 × 리턴"의 기대값으로 바꾸는 **로그-미분 트릭**이 핵심입니다.
...
상태에만 의존하는 베이스라인 b(s)를 빼도 기대값은 불변이지만 분산은 크게 줄어듭니다.\nb(s) = V(s)를 쓰면 (G_t − V(s))는 "그 행동이 평균보다 얼마나 좋았나"를 뜻하는 **어드밴티지**가 됩니다 — 다음 교시 Actor-Critic의 핵심 아이디어입니다.`,
        },
      ],
      code: {
...
1일차의 "MC → TD" 전환을 정책 경사에 그대로 적용한 것입니다.`,
        ...
        {
          h: '어드밴티지와 A2C',
          p: `어드밴티지 A(s,a) = Q(s,a) − V(s)는 "이 행동이 그 상태의 평균보다 얼마나 좋은가"입니다.\n실전에서는 ==TD 오차 δ = r + γV(s') − V(s)가 어드밴티지의 불편추정치==이므로 Critic 하나로 충분합니다.
