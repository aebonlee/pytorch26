  ...
  sessions: [
    {
      ...
      theory: [
        {
          h: '강화학습이란?',
          p: `강화학습(Reinforcement Learning)은 에이전트(Agent)가 환경(Environment)과 상호작용하며 보상(Reward)을 최대화하는 행동 전략을 스스로 학습하는 기계학습 패러다임입니다.
지도학습처럼 "정답 레이블"이 주어지지 않고, 비지도학습처럼 구조만 찾는 것도 아닙니다. 행동의 결과로 주어지는 평가적(evaluative) 피드백만으로 학습한다는 점이 핵심입니다.
...
가장 단순한 해법이 ε-greedy: 확률 ε로는 무작위 행동(탐험), 1-ε로는 현재 최선의 행동(활용)을 선택합니다. 이 딜레마는 3일 내내 모든 알고리즘에서 반복해서 등장합니다.`,
        },
      ],
      code: {
...
# 0.5(과도한 탐험)는 보상을 낭비합니다. 0.1 근처가 균형점.`,
    ...
    },
    {
      ...
      theory: [
        {
          h: '마르코프 결정 과정 (MDP)',
          p: `강화학습 문제를 수학적으로 정의하는 틀이 MDP입니다. 5개 요소로 구성됩니다.
· S: 상태 집합 · A: 행동 집합 · P(s'|s,a): 상태 전이 확률 · R(s,a): 보상 함수 · γ: 할인율 (0~1)
핵심 가정은 마르코프 성질 — "미래는 현재 상태에만 의존하고 과거 경로와는 무관하다"입니다. 체스에서 지금 기물 배치만 알면 과거 수순은 몰라도 되는 것과 같습니다.`,
        },
        {
          h: '가치함수 — V와 Q',
...
"지금 상태의 가치 = 즉시 보상 + 할인된 다음 상태의 가치"라는 이 한 줄이 강화학습 전체를 관통합니다. 최적 가치함수에 대한 벨만 최적 방정식은 V*(s) = max_a Σ_s' P(s'|s,a)[R + γV*(s')] 형태가 됩니다.`,
        },
      ],
      code: {
...
정책 개선: 계산된 V^π를 이용해 각 상태에서 greedy 행동을 취하는 새 정책 π'를 만듭니다. 정책 개선 정리에 의해 π'는 π보다 항상 같거나 좋습니다.
이 둘을 번갈아 반복하면(일반화된 정책 반복, GPI) 최적 정책에 도달합니다.`,
        },
        {
          h: '정책 반복 vs 가치 반복',
          p: `정책 반복(PI): 평가를 완전히 수렴시킨 뒤 개선. 반복 횟수는 적지만 한 번의 평가가 무겁습니다.
가치 반복(VI): 평가를 1스텝만 하고 바로 max를 취해 개선을 겸합니다. V(s) ← max_a Σ P[R + γV(s')]. 구현이 간단하고 실전에서 많이 쓰입니다.
...
print("두 가치함수 일치:", np.allclose(pi_V, vi_V))`,
    ...
    },
    {
      ...
      theory: [
        {
          h: '모델을 모를 때 — 샘플로 배우기',
          p: `DP는 전이확률 P를 알아야 합니다. 현실에선 모르는 경우가 대부분이므로, 실제로 행동해 보고 얻은 경험 샘플로 가치를 추정해야 합니다. 대표적인 두 방법이 몬테카를로(MC)와 시간차(TD) 학습입니다.`,
        },
        {
          h: '몬테카를로(MC) 방법',
...
괄호 안을 TD 오차(δ)라고 부릅니다. 매 스텝 학습이 가능하고 분산이 작지만, 부정확한 추정치로 추정치를 갱신하므로 편향이 생깁니다. 이 bias-variance 트레이드오프가 MC vs TD 선택의 핵심입니다.`,
        },
      ],
      code: {
...
# 둘 다 DP 정답(-14, -20, -22...)에 근접하는지 확인하세요`,
    ...
    },
    {
      ...
      theory: [
        {
          h: '예측에서 제어로',
          p: `지금까지는 주어진 정책의 가치를 "예측"했습니다. 이제 최적 정책을 직접 "찾는" 제어(control) 문제로 넘어갑니다. 모델이 없으므로 V 대신 Q(s,a)를 학습해야 greedy 정책을 만들 수 있습니다.`,
        },
        {
          h: 'SARSA — On-policy TD 제어',
          p: `Q(s,a) ← Q(s,a) + α [ r + γQ(s',a') − Q(s,a) ]
이름 그대로 (S, A, R, S', A') 5개를 사용합니다. 다음 행동 a'를 실제 행동하는 정책(ε-greedy)에서 뽑으므로, "행동하는 정책"과 "학습하는 정책"이 같은 on-policy 방법입니다. 탐험의 위험까지 Q값에 반영되어 보수적인 경로를 학습합니다.`,
        },
        {
          h: 'Q-Learning — Off-policy TD 제어',
          p: `Q(s,a) ← Q(s,a) + α [ r + γ max_a' Q(s',a') − Q(s,a) ]
다음 상태에서 실제 취할 행동과 무관하게 max를 사용합니다. 행동은 ε-greedy로 하되 학습 목표는 greedy 정책 — 행동 정책과 목표 정책이 다른 off-policy 방법입니다.
Cliff Walking 예제에서 SARSA는 절벽에서 먼 안전한 길을, Q-Learning은 절벽에 붙은 최단 경로를 배우는 차이가 이 구조에서 나옵니다. Q-Learning의 max 구조는 2일차 DQN으로 직결됩니다.`,
        },
      ],
      code: {
...
관찰 포인트: 학습 곡선에서 SARSA의 에피소드 보상이 더 높게(안전한 길) 나오고, Q-Learning은 탐험 중 절벽에 자주 떨어져 보상이 낮지만 최종 greedy 정책은 최단 경로가 됩니다.`,
        },
      ],
      code: {
