...
//   blanks: [{ level: 1~3, find, hint, nth? }]
//     — 괄호채우기 빈칸 스펙. find는 solution.source의 정답 코드
//       조각(정확 일치 필수), 난이도 L 선택 시 level<=L 누적 적용.
//       utils/skeleton.js가 런타임에 빈칸 코드를 생성한다.
//   solution: { filename, source }  // 완성형 코드 — sol0N
// }  괄호채우기/완성형은 카드 우하단 버튼 2개로 각각 토글 공개.
// ※ solution 코드를 수정하면 blanks의 find도 함께 맞출 것 —
//   find가 어긋나면 그 빈칸만 조용히 사라진다.
...
export default [
  {
    ...
    blanks: [
      { level: 1, find: 'r + gamma * Q[s2].max() * (not done) - Q[s][a]', hint: 'Q-Learning TD 오차: 목표(r + γ·다음상태 최대Q·(not done)) − 현재 Q' },
      { level: 2, find: 'int(Q[s].argmax())', hint: 'greedy 행동 — Q[s]에서 최대인 행동' },
      { level: 2, find: 'max(eps_min, eps * eps_decay)', hint: 'ε 감쇠 — eps_min 아래로 안 내려가게' },
      { level: 3, find: 'np.zeros((nS, nA))', hint: 'Q 테이블 초기화 — 상태 수 × 행동 수' },
      { level: 3, find: 'arrows[Q.argmax(axis=1)].reshape(4, 4)', hint: '상태별 최적 행동을 화살표로 바꿔 4x4로' },
    ],
    solution: {
...
# 화살표가 오히려 최적일 수 있습니다 — 왜인지 생각해 보세요`,
  ...
  },
  {
    ...
    blanks: [
      { level: 1, find: 'Q[s2][a2] if method == "sarsa" else Q[s2].max()', hint: '두 알고리즘의 유일한 차이 — SARSA=실제 다음 행동의 Q / QL=최대 Q' },
      { level: 2, find: 'r + gamma * nxt * (not done) - Q[s][a]', hint: 'TD 업데이트 — 목표 − 현재 추정치' },
      { level: 3, find: 'int(rng.integers(nA)) if rng.random() < eps else int(Q[s].argmax())', hint: 'ε-greedy 행동 선택' },
      { level: 3, find: 'np.convolve(x, np.ones(k) / k, "valid")', hint: '100 에피소드 이동평균' },
    ],
    solution: {
...
# 둘 다 8 이상이면 인증! eps=0.3으로 올려 재실험해 보세요`,
  ...
  },
  {
    ...
    blanks: [
      { level: 1, find: 'q(s2).argmax(1, keepdim=True)', hint: '"선택" — 온라인넷으로 s2의 argmax' },
      { level: 1, find: 'q_t(s2).gather(1, best).squeeze(1)', hint: '"평가" — 타깃넷에서 best 행동의 Q' },
      { level: 2, find: 'r + gamma * nxt * (1 - d)', hint: 'TD 목표 — 종료(d=1)면 미래항 0' },
      { level: 2, find: 'q_t.load_state_dict(q.state_dict())', hint: '타깃 네트워크 동기화 — 온라인넷 가중치 복사', nth: 2 },
      { level: 3, find: 'q(s).gather(1, a.unsqueeze(1)).squeeze(1)', hint: '배치에서 \'선택했던 행동\'의 Q만 추출' },
      { level: 3, find: 'nn.functional.smooth_l1_loss(qv, y)', hint: 'Huber 손실 (smooth L1)' },
    ],
    solution: {
...
for ep in range(800):
  ...
  },
  {
    ...
    blanks: [
      { level: 1, find: 'r + 0.99 * R * (1 - d)', hint: 'n-step 리턴 누적 — 종료(d)면 미래항 리셋' },
      { level: 2, find: 'targets - vals', hint: '어드밴티지 = TD 목표 − 가치 추정' },
      { level: 2, find: '-(torch.stack(logps) * adv.detach()).mean()', hint: 'Actor 손실 — logπ × 어드밴티지(detach!)' },
      { level: 3, find: '0.5 * adv.pow(2).mean()', hint: 'Critic 손실 — 어드밴티지 제곱 평균 × 0.5' },
      { level: 3, find: 'ent_coef * torch.stack(ents).mean()', hint: '엔트로피 보너스 — 탐험 유지' },
    ],
    solution: {
...
# ent=0 조합이 유독 낮다면 조기 수렴(탐험 소멸)을 관찰한 것입니다`,
  ...
  },
  {
    ...
    blanks: [
      { level: 1, find: 'br + 0.99 * critic_t(bs2, actor_t(bs2)) * (1 - bd)', hint: 'TD 목표 — 반드시 타깃 네트워크들로!' },
      { level: 2, find: '-critic(bs, actor(bs)).mean()', hint: 'Actor 손실 — Q(s, μ(s)) 최대화 (부호 주의)' },
      { level: 3, find: 'soft_update(actor_t, actor); soft_update(critic_t, critic)', hint: '두 타깃 네트워크 소프트 업데이트' },
    ],
    solution: {
...
# noise_std를 0.05/0.1/0.3으로 바꿔 DDPG 민감도도 확인해 보세요`,
  ...
  },
  {
    ...
    blanks: [
      { level: 1, find: '(prob.pow(q - 1) - 1) / (q - 1)', hint: 'Tsallis q-logarithm 정의' },
      { level: 2, find: 'log_prob.exp().clamp(min=1e-8)', hint: '로그확률 → 확률 복원 (0 방지 클램프)' },
      { level: 3, find: 'abs(np.mean(r_sac[-20:]) - np.mean(r_q1[-20:]))', hint: '회귀 테스트 지표 — 마지막 20ep 평균 차이' },
    ],
    solution: {
...
# plt.legend(); plt.show()`,
  ...
  },
  {
    ...
    blanks: [
      { level: 1, find: '300.0 * (gamma * abs(s2[1]) - abs(s[1]))', hint: '속도 기반 potential shaping — 빨라질수록 보너스' },
      { level: 2, find: '100.0 * float(err.detach())', hint: 'curiosity 내적 보상 — 예측오차(놀라움) × 스케일' },
      { level: 3, find: 'r + gamma * q_t(s2).max(1).values * (1 - d)', hint: 'TD 목표 (타깃넷 사용)' },
    ],
    solution: {
...
# MODE="none"으로 바꿔 다시 돌리면 왜 실패하는지 체감할 수 있습니다`,
  ...
  },
  {
    ...
    blanks: [
      { level: 1, find: '10.0 if caught else -0.1', hint: '보상 설계 — 잡으면 +10, 스텝 페널티 -0.1' },
      { level: 2, find: 'spaces.Box(0, size - 1, shape=(4,), dtype=np.float32)', hint: '관측 공간 — 0~size-1 실수 4개' },
      { level: 2, find: 'spaces.Discrete(4)', hint: '행동 공간 — 상하좌우 4개' },
      { level: 3, find: '0.1 * (r + 0.99 * Q[k2].max() * (not done) - Q[k][a])', hint: 'Q-Learning 업데이트 (α=0.1, γ=0.99)' },
    ],
    solution: {
