// ============================================================
// 실습소스 모음 (/labs) — 일자별 탭 구분
// ------------------------------------------------------------
// 21교시 커리큘럼에 흩어져 있는 실습 코드를 모아 DAY 1/2/3
// 탭으로 나눠 보여준다 (2026-07-17 대표 지시로 목록 나열 → 탭 전환).
// 사이드바 책갈피는 현재 탭(일차)의 교시들만 표시.
// 데이터를 따로 두지 않고 curriculum.js의 days에서 파생하므로,
// 교시 코드를 수정하면 여기에도 자동 반영된다.
// ============================================================
...
export default function LabsPage() {
  ...
  const codeSessions = day.sessions.filter((s) => s.code)
  const totalCodes = days.reduce((n, d) => n + d.sessions.filter((s) => s.code).length, 0)

  // 책갈피: 현재 탭 일차의 코드 교시들
  const navItems = codeSessions.map((s) => ({
    key: `d${day.id}s${s.slot}`,
    anchor: `labs-d${day.id}s${s.slot}`,
  ...
  return (
    <div className="container page-side">
      ...
      <div className="session-head">
        <h1>실습소스 모음</h1>
        <p className="meta">
          3일 과정의 실습 코드 {totalCodes}개를 일자별 탭으로 정리했습니다 —
      ...
      <div className="objectives">
        ...
        <ul>
          ...
          <li>즉, 각 일차 안에서는 위에서부터 순서대로 실행하는 것을 전제로 작성되어 있습니다</li>
        </ul>
      </div>
      ...
      <div className="tab-row">
        {days.map((d) => (
          <button
            key={d.id}
            type="button"
            className={`tab ${d.id === activeDay ? 'on' : ''}`}
            onClick={() => { setActiveDay(d.id); window.scrollTo({ top: 0 }) }}
          >
            DAY {d.id} · {d.theme}
            <span className="cnt">코드 {d.sessions.filter((s) => s.code).length}개</span>
          </button>
        ))}
      </div>
